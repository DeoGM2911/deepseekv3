...
from loss import compute_load_balancing_loss
    ...
    def train_epoch(self):
        ...
        for batch in pbar:
            ...
            with autocast(self.config.device.type, dtype=self.config.precision):
                ...
                moe_loss = compute_load_balancing_loss(moe_logits_list, moe_topk_indices, self.config.num_experts)
                
                # Reshape for CrossEntropyLoss: (Batch * Seq, Vocab) vs (Batch * Seq)
                vocab_size = outputs.size(-1)
                loss = self.criterion(outputs.view(-1, vocab_size), labels.view(-1)) + moe_loss
